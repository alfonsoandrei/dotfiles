# Generated by Powerlevel10k configuration wizard on 2026-04-17 at 08:29 EDT.
...
# round separators, round heads, flat tails, 2 lines, disconnected, left frame,
...
() {
  ...
  function my_git_formatter() {
    ...
    local conflicted='%15F' # bright white foreground

    local res

    if [[ -n $VCS_STATUS_LOCAL_BRANCH ]]; then
      local branch=${(V)VCS_STATUS_LOCAL_BRANCH}
      # Collapse slash-separated path components to first letter except the last segment.
      # e.g. feature/some-team/my-branch -> f/s/my-branch
      if [[ $branch == */* ]]; then
        local -a parts=("${(@s:/:)branch}")
        local i
